...
new:mainbutton("击杀NPC", "立即杀死NPC", function()
	if currentnpc then
...
new:mainbutton("传送NPC", "将NPC传送到你的位置", function()
	if currentnpc then
		local part = currentnpc:FindFirstChild("HumanoidRootPart")
		if part and part.ReceiveAge == 0 then
...
new:mainbutton("前往NPC", "传送你的角色到NPC位置", function()
	if currentnpc then
		local part = currentnpc:FindFirstChild("HumanoidRootPart")
		if part then
			if lp and lp.Character then
...
local chr
new:maintoggle("控制NPC", "控制NPC/假人直到NPC失去网络所有权", function(a)
	if a then
		if currentnpc then
			local part = currentnpc:FindFirstChild("HumanoidRootPart")
			if part and part.ReceiveAge == 0 then
...
new:mainbutton("惩罚NPC", "让NPC消失到太空中", function()
	if currentnpc then
		local part = currentnpc:FindFirstChild("HumanoidRootPart")
		if part and part.ReceiveAge == 0 then
			if lp and lp.Character then
...
new:mainbutton("NPC坐下状态", "让NPC保持坐姿", function()
	if currentnpc then
		local part = currentnpc:FindFirstChild("HumanoidRootPart")
		if part and part.ReceiveAge == 0 then
			local hum = currentnpc:FindFirstChildOfClass("Humanoid")
...
new:mainbutton("NPC跳跃状态", "让NPC跳跃", function()
	if currentnpc then
		local part = currentnpc:FindFirstChild("HumanoidRootPart")
		if part and part.ReceiveAge == 0 then
			local hum = currentnpc:FindFirstChildOfClass("Humanoid")
...
local con, follownpc
follownpc = new:maintoggle("NPC跟随", "让NPC跟随你", function(a)
	if a then
...
local con1
new:extratoggle("NPC击杀光环", function(a)
	if a then
